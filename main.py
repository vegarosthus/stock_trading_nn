...
if __name__ == '__main__':
	...
	requested_stocks = ['STL', 'REC', 'SDRL', 'SAS-NOK', 'ATLA-NOK', 'BIONOR', 'HAVI', 'SBX', 'FUNCOM', 'TEL', 'ARCHER', 'NAVA', 'EMGS', 'MHG', 'FRO']
	...
	NN = nn.NeuralNetwork(FEATURES*INTERVAL, 7, 7)
	...
	X_p, stocks, y_p, promising_stocks = NN.predict(X_p, stocks)
...
#	
	plot_learning_curve(costs_train, costs_cv, costs_test, accuracies_train, f1s_train, accuracies_cv, f1s_cv, accuracies_test, f1s_test)
#	
	for stock in promising_stocks:
		print(stock.name)

#stock_algorithms.trade(OSEBX, my_portfolio, 'historic', 0)
...
#		returns = OSEBX.get_returns(requested_stocks, N, interval)
#
#		# Get volume movements over last two days
#		volume_movement = np.concatenate((OSEBX.get_volume_incs(requested_stocks), np.zeros((1,6))[0]))#np.array(OSEBX.get_volume_incs(requested_stocks))#
#
#		# Get recent second derivatives
#		dds = np.concatenate((OSEBX.get_dds(requested_stocks), np.zeros((1,6))[0]))#np.array(OSEBX.get_dds(requested_stocks))#
#
#		# Get ratio of moving average and lower bollinger band for stocks
#		sma_bb_ratio = np.concatenate((OSEBX.get_sma_bb_ratios(requested_stocks), np.zeros((1,6))[0]))#np.array(OSEBX.get_sma_bb_ratios(requested_stocks))#
#
#		# Get historic stock variances
#		cov_matrix = OSEBX.get_stock_covariances(requested_stocks, N, interval)
#
#		filter = OSEBX.filter(requested_stocks, promising_stocks)
#
#		print(filter)
#
#		# Generate optimization problem
#		P, q, G, h, A, b, means, stds = opt.generate_ProblemMatrices(returns, cov_matrix, sma_bb_ratio, volume_movement, filter, rho, phi, kappa)
#
#		# Solve optimization problem
#		optimized_portfolio = opt.optimize_portfolio(OSEBX, requested_stocks, P, q, G, h, A, b, means, stds, sma_bb_ratio, mu)
#
